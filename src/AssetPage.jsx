...
function ChecklistView({ asset, userRole, onDone, onBack }) {
  ...
  const complete = async () => {
    ...
    await supabase.from("maintenance_logs").insert([{ id: uid("LOG"), asset_id: asset.id, asset_name: asset.name, log_type: "Preventive Maintenance", title: `CIL Checklist — ${new Date().toLocaleString("default", { month: "long", year: "numeric" })}`, description, performed_by: executedBy, vendor: null, start_date: TODAY, end_date: TODAY, cost: null, status: needsApproval ? "In Progress" : "Completed", approval_status: needsApproval ? "Pending" : "Approved" }]);
