...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [policiesRes, linksRes] = await Promise.all([
      fetch(
      ...
      fetch(
        `${supabaseUrl}/rest/v1/insurance_policy_assets?select=policy_id,asset_name`,
        { headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` } }
      ),
    ]);
    ...
    const assetNamesByPolicy = {};
    if (Array.isArray(links)) {
      for (const l of links) {
        if (!assetNamesByPolicy[l.policy_id]) assetNamesByPolicy[l.policy_id] = [];
        assetNamesByPolicy[l.policy_id].push(l.asset_name);
      }
    }

    const rows = policies.map((p) => {
      const expired = p.expiry_date < todayStr;
      const names = assetNamesByPolicy[p.id] || [];
      const assetsLabel = names.length > 3 ? `${names.slice(0,3).join(", ")} +${names.length-3} more` : (names.join(", ") || "—");
      return `<tr>
        <td style="padding:8px;border-bottom:1px solid #e5e7eb;">${p.provider || "—"} ${p.policy_number ? `(${p.policy_number})` : ""}</td>
        <td style="padding:8px;border-bottom:1px solid #e5e7eb;">${assetsLabel}</td>
        ...
        <div style="background: #f9fafb; padding: 20px; border-radius: 0 0 8px 8px;">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead><tr style="text-align:left;background:#f3f4f6;">
              <th style="padding:8px;">Provider / Policy #</th><th style="padding:8px;">Covered Assets</th><th style="padding:8px;">Expiry</th>
            </tr></thead>
            <tbody>${rows}</tbody>
          </table>
